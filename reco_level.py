...
files = {
'pp_hhjj_bbbbjj':'/lstore/calo/lcoelho/MG_samples/VBF_hh_loop_sm/Events/run_01_LO_decayed_1/output_Delphes_90btag.root',
}
...
TH1D_ncut = ROOT.TH1D("ncut", "ncut", 6, 0, 6)
TH1D_ncut.Sumw2()

TH1D_PTb1 = ROOT.TH1D("PT_b1", "P_{T}(b_{1})", 20, 0, 400)
TH1D_PTb1.Sumw2()

TH1D_PTb2 = ROOT.TH1D("PT_b2", "P_{T}(b_{2})", 20, 0, 400)
TH1D_PTb2.Sumw2()

TH1D_PTb3 = ROOT.TH1D("PT_b3", "P_{T}(b_{3})", 20, 0, 400)
TH1D_PTb3.Sumw2()

TH1D_PTb4 = ROOT.TH1D("PT_b4", "P_{T}(b_{4})", 20, 0, 400)
TH1D_PTb4.Sumw2()

TH1D_PTj1 = ROOT.TH1D("PT_j1", "P_{T}(j_{1})", 20, 0, 400)
TH1D_PTj1.Sumw2()

TH1D_PTj2 = ROOT.TH1D("PT_j2", "P_{T}(j_{2})", 20, 0, 400)
TH1D_PTj2.Sumw2()

TH1D_Mjj = ROOT.TH1D("M_jj", "M(j_{1}j_{2})", 20, 0, 2500)
TH1D_Mjj.Sumw2()

TH1D_Mh1 = ROOT.TH1D("M_h1", "M(h_{1})", 20, 0, 400)
TH1D_Mh1.Sumw2()

TH1D_Mh2 = ROOT.TH1D("M_h2", "M(h_{2})", 20, 0, 400)
TH1D_Mh2.Sumw2()

TH1D_Mhh = ROOT.TH1D("M_hh", "M(hh)", 20, 0, 1500)
TH1D_Mhh.Sumw2()

TH1D_PTh1 = ROOT.TH1D("PT_h1", "P_{T}(h_{1})", 20, 0, 400)
TH1D_PTh1.Sumw2()

TH1D_PTh2 = ROOT.TH1D("PT_h2", "P_{T}(h_{2})", 20, 0, 400)
TH1D_PTh2.Sumw2()

TH1D_Etah1 = ROOT.TH1D("Eta_h1", "#eta(h_{1})", 20, -10, 10)
TH1D_Etah1.Sumw2()

TH1D_Etah2 = ROOT.TH1D("Eta_h2", "#eta(h_{2})", 20, -10, 10)
TH1D_Etah2.Sumw2()

TH1D_Phih1 = ROOT.TH1D("Phi_h1", "#Phi(h_{1})", 20, -6, 6)
TH1D_Phih1.Sumw2()

TH1D_Phih2 = ROOT.TH1D("Phi_h2", "#Phi(h_{2})", 20, -6, 6)
TH1D_Phih2.Sumw2()

TH1D_deltaEtahh = ROOT.TH1D("deltaEtahh", "#Delta #eta(h_{1}h_{2})", 20, -10, 10)
TH1D_deltaEtahh.Sumw2()

TH1D_deltaPhihh = ROOT.TH1D("deltaPhihh", "#Delta #Phi(h_{1}h_{2})", 20, -6, 6)
TH1D_deltaPhihh.Sumw2()

TH1D_bb1_bb2_angle = ROOT.TH1D("bb1_bb2_angle", "#Theta^{Lab}(b #bar{b}_{1} V b #bar{b}_{2})", 20, 0, 3.14159266)
TH1D_bb1_bb2_angle.Sumw2()

TH1D_hh_jj_angle = ROOT.TH1D("hh_jj_angle", "#Theta^{Lab}(hh V jj)", 20, 0, 3.14159266)
TH1D_hh_jj_angle.Sumw2()

TH1D_jj_bb1_angle = ROOT.TH1D("jj_bb1_angle", "#Theta^{Lab}(jj V b #bar{b}_{1})", 20, 0, 3.14159266)
TH1D_jj_bb1_angle.Sumw2()

TH1D_jj_bb2_angle = ROOT.TH1D("jj_bb2_angle", "#Theta^{Lab}(jj V b #bar{b}_{2})", 20, 0, 3.14159266)
TH1D_jj_bb2_angle.Sumw2()

TH1D_bb1_bb2_angle_hhframe = ROOT.TH1D("bb1_bb2_angle_hhframe", "#Theta^{hh}(b #bar{b}_{1} V b #bar{b}_{2})", 20, 0, 3.14159266)
TH1D_bb1_bb2_angle_hhframe.Sumw2()

TH1D_hh_jj_angle_hhjjframe = ROOT.TH1D("hh_jj_angle_hhjjframe", "#Theta^{hhjj}(hh V jj)", 20, 0, 3.14159266)
TH1D_hh_jj_angle_hhjjframe.Sumw2()

TH1D_jj_bb1_angle_hhframe = ROOT.TH1D("jj_bb1_angle_hhframe", "#Theta^{hh}(jj V b #bar{b}_{1})", 20, 0, 3.14159266)
TH1D_jj_bb1_angle_hhframe.Sumw2()

TH1D_jj_bb2_angle_hhframe = ROOT.TH1D("jj_bb2_angle_hhframe", "#Theta^{hh}(jj V b #bar{b}_{2})", 20, 0, 3.14159266)
TH1D_jj_bb2_angle_hhframe.Sumw2()

TH1D_j1hh_hh_angle = ROOT.TH1D("j1hh_hh_angle", "#Theta^{j1hh}_{hh}", 20, 0, 3.14159266)
TH1D_j1hh_hh_angle.Sumw2()

TH1D_hh_h1_angle = ROOT.TH1D("hh_h1_angle", "#Theta^{hh}_{h1}", 20, 0, 3.14159266)
TH1D_hh_h1_angle.Sumw2()

TH1D_hh_h2_angle = ROOT.TH1D("hh_h2_angle", "#Theta^{hh}_{h2}", 20, 0, 3.14159266)
TH1D_hh_h2_angle.Sumw2()

TH1D_cos_Y_h1 = ROOT.TH1D("cos_Y_h1", "|cos(#Theta^{j1hh}_{hh})cos(#Theta^{hh}_{h1})|", 20, 0, 1)
TH1D_cos_Y_h1.Sumw2()

TH1D_cos_Y_h2 = ROOT.TH1D("cos_Y_h2", "|cos(#Theta^{j1hh}_{hh})cos(#Theta^{hh}_{h2})|", 20, 0, 1)
TH1D_cos_Y_h2.Sumw2()
...
for entry in tree_hh:
    ...
    for jet_index in range(number_of_jets):
        ...
        if tree_hh.GetLeaf("Jet.BTag").GetValue(jet_index) == True and jet_PT > 40 and abs(jet_Eta) < 2 and len(bjet_list) != 4:
        #if tree_hh.GetLeaf("Jet.BTag").GetValue(jet_index) == True: 
            bjet_list.append(jet)

        elif jet_PT > 30 and abs(jet_Eta) > 2:
        #else:
            jet_list.append(jet)  
    ...
    if number_of_btagged_jets == 0:
        TH1D_ncut.Fill(0)
    if number_of_btagged_jets == 1:
        TH1D_ncut.Fill(1)
    if number_of_btagged_jets == 2:
        TH1D_ncut.Fill(2)
    if number_of_btagged_jets == 3:
        TH1D_ncut.Fill(3)
    if number_of_btagged_jets == 4:
        TH1D_ncut.Fill(4)
    if number_of_btagged_jets >= 5:
        TH1D_ncut.Fill(5)
    ...
    # Two highest-pT jets with opposite Eta sign 
    if np.sign(j1.Eta()) == np.sign(j2.Eta()):
        continue

    #TH1D_ncut.Fill(2)

    # Check if |delta Eta(jj)| > 5 and m(jj) > 1000
    deltaEta_jj = abs(j1.Eta() - j2.Eta())
    if deltaEta_jj <= 5 or abs(jj_pair.M()) <= 1000:
        continue
    ...
    # m(4b) selections
    bbbb_M = abs(bbbb_jets.M())
    deltaEta_bb_lead = abs(b11_best.Eta() - b12_best.Eta())
    deltaEta_bb_subl = abs(b21_best.Eta() - b22_best.Eta())
    if bbbb_M < 1250:
        if deltaEta_bb_lead >= 0.475+653.0/bbbb_M or deltaEta_bb_lead <= 360/bbbb_M-0.5:
            continue
        if deltaEta_bb_subl >= 0.35+875.0/bbbb_M or deltaEta_bb_subl <= 235/bbbb_M:
            continue
    if bbbb_M >= 1250:
        if deltaEta_bb_lead >= 1 or deltaEta_bb_subl >= 1:
            continue
...
outHistFile = ROOT.TFile.Open("plots/reco_level.root" ,"RECREATE")
outHistFile.cd()
for hist in TH1D_list:
    hist.Write()
outHistFile.Close()
...
for hist_i, hist in enumerate(TH1D_list):
    hist.Draw('h')
    canvas.Print('plots/reco_cut_level_'+variables_lsit[hist_i]+'.pdf')
